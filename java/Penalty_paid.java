 ...
 * @author student
 */
public class Penalty_paid extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Penalty_paid</title><style>\n" +
"            body {\n" +
"              margin: 0;\n" +
"            }\n" +
"\n" +
...
"        </div>");
            ...
            try {
                ...
                if(rs.next())
                {
                    ...
                    out.println("<TH>select to dismiss penalty</TH>");
                    out.println("</TR>");
                    // the data 
                
                    out.println("<form  method=\"POST\" action=\"Penalty_paid_confirm\" name=\"paid\">");
                    do {
                         out.println("<TR>");
                         for (int i = 0; i < columnCount; i++) {
                           out.println("<TD>" + rs.getString(i + 1) + "</TD>");
                           }
                        out.println("<TD><input type=\"radio\" name=\"penalty_id\" value="+rs.getInt("penalty_id")+" checked></TD>");
                        out.println("</TR>");

                    }while (rs.next());
                    out.println("<center><input type=\"submit\" value=\"click here to dismiss selected penalty\"></form>");
                }
                else
                    out.println("No penalties to be paid");
