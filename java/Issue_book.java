...
public class Issue_book extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Issue_book</title><style>\n" +
"            body {\n" +
"              margin: 0;\n" +
"            }\n" +
"\n" +
...
"        </div>");
            ...
            try {
                ...
                if(rs.next())
                {
                    ...
                    out.println("<TH>select to issue</TH>");
                    out.println("</TR>");
                
                // the data 
                
                    out.println("<form  method=\"POST\" action=\"Issue_book_confirm\" name=\"issue\">");
                    do {
                         out.println("<TR>");
                         for (int i = 0; i < columnCount; i++) {
                           out.println("<TD>" + rs.getString(i + 1) + "</TD>");
                           }
                        out.println("<TD><input type=\"radio\" name=\"req_id\" value="+rs.getInt("req_id")+" checked></TD>");
                        out.println("</TR>");

                    }while (rs.next());
                    out.println("<center><input type=\"submit\" value=\"click here to issue selected request\"></form>");
                }
                else
                    out.println("No requests to be issued");
